...
import AdminHotelSettingsForm from "../../components/admin/settings/AdminHotelSettingsForm";
...
const AdminSettingsPage = () => {
  ...
  const fetchHotel = async () => {
    try {
      setLoading(true);
      const data = await adminHotelApi.getMyHotel();
      setHotel(data);
    } catch (err) {
  ...
  const handleSubmit = async (data) => {
    try {
      await adminHotelApi.updateHotel(data);
  ...
  return (
    <div className="business-settings-page">
      ...
      <div className="card">
        <AdminHotelSettingsForm hotel={hotel} onSubmit={handleSubmit} />
      </div>
